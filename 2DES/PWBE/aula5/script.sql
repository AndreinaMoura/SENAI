...
select id_func, funcionarios.nome as nome_funcionarios, funcionarios.matricula, id_prod, produtos.nome as nome_produtos, produtos.preco, sum(vendas.qntd) as total_quantidade from vendas 
inner join funcionarios on vendas.id_func = funcionarios.id 
inner join produtos on vendas.id_prod = produtos.id 
group by id_func, id_prod;
